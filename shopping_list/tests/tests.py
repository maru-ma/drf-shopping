...
def test_shopping_list_name_missing_returnsbad_request():
    url = reverse("all-shopping-lists")
    data = {
        "shomething_else": "ljksdlaskjd"
    }
    client = APIClient()
    response = client.post(url, data, format="json")
...
def test_all_shopping_lists_are_listed():
    ...
    ShoppingList.objects.create(name="Bath")
    ...
    assert response.data[1]["name"] == "Bath"
